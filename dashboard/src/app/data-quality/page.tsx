    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="glass-card-static p-5 animate-slide-up" style={{ border: expandedKpi === "failed" ? "1px solid rgba(239,68,68,0.15)" : "1px solid rgba(16,185,129,0.15)" }}>
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-white">{expandedKpi === "failed" ? "Failed Checks Detail" : "Passed Checks Detail"}</h4>
                        ...
                        <div className="space-y-2">
                            {kpiDrilldowns[expandedKpi].items.map((item, i) => (
                                <div key={i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/[0.02] transition-colors" style={{ border: "1px solid rgba(255,255,255,0.04)" }}>
                                    {item.status === "pass" ? (
            ...
            <div id="checks" className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                        ...
                        {checkResults.map((check: any, idx: number) => {
                            ...
                            return (
                                <div key={check.check_id}>
                                    <div
                                        className={`flex items-center gap-3 p-3 rounded-xl transition-all border border-white/[0.03] ${isExpanded ? "bg-white/[0.03]" : "hover:bg-white/[0.02]"
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex gap-1.5 mt-1">
                                                {(check.datasets || []).map((ds: string) => (
                                                    <span key={ds} className="text-[9px] text-slate-600 bg-white/[0.03] px-1.5 py-0.5 rounded">{datasetIcons[ds] || "📊"} {ds.replace(/_/g, " ")}</span>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <table className="w-full">
                                                    ...
                                                    <tbody>
                                                        {violations.slice(0, 10).map((v: any, vi: number) => (
                                                            <tr key={vi} className="border-t border-white/[0.03]">
                                                                <td className="px-3 py-2 text-xs text-slate-400">{datasetIcons[v.source] || ""} {v.source}</td>
                                                                <td className="px-3 py-2 text-xs text-white font-mono">{v.row_id || `row ${v.row_index}`}</td>
                ...
                <div className="space-y-4">
                    ...
                    <ChartCard title="Overall Quality Score" subtitle="Latest pipeline run" className="animate-slide-up">
                        ...
                        {/* Pass/Fail mini bar */}
                        <div className="mt-2">
                            <div className="flex items-center gap-1 h-3 rounded-full overflow-hidden mb-2" style={{ background: "rgba(255,255,255,0.04)" }}>
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <p className="text-sm font-semibold text-white capitalize">{ds.layer.replace(/_/g, " ")}</p>
                                        </div>
            ...
            {Object.keys(columnsBySource).length > 0 && (
                <div id="completeness">
                    <ChartCard title="Column Completeness" subtitle="Non-null percentage per column across all datasets" className="animate-slide-up">
                        <div className="space-y-6">
                            {Object.entries(columnsBySource).map(([source, cols]: [string, any[]]) => (
                                <div key={source}>
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="text-base">{datasetIcons[source] || "📊"}</span>
                                        <h4 className="text-xs font-bold text-white uppercase tracking-wider">{source.replace(/_/g, " ")}</h4>
                                        <span className="text-[10px] text-slate-600 ml-auto">{cols.length} columns</span>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2">
                                        {cols.map((col: any) => {
                                            const pct = col.completeness_pct;
                                            const isFull = pct >= 100;
                                            const isLow = pct < 95;
                                            return (
                                                <div key={col.column} className="flex items-center gap-3 p-2.5 rounded-lg" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${isLow ? "rgba(239,68,68,0.15)" : "rgba(255,255,255,0.04)"}` }}>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-xs font-medium text-white font-mono truncate">{col.column}</p>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                                                                <div
                                                                    ...
                                                                    style={{
                                                                        width: `${pct}%`,
                                                                        background: isFull ? "#10b981" : isLow ? "#ef4444" : "#f59e0b",
                                                                    }}
                                                                />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <span className={`text-xs font-bold tabular-nums ${isFull ? "text-emerald-400" : isLow ? "text-red-400" : "text-amber-400"}`}>
                                                        {pct}%
                                                    </span>
                                                </div>
                                            );
